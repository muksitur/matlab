...
%this website has the similar problem defined. https://github.com/mahu/IAOR/blob/master/Exercise%206/LearnGaussMixModel.m
...
% IMPLEMENT THIS FUNCTION (TASK A.b)
    ...
    denominators = [];
    for feature = 1:size(lnVectorProb,2)

        x = lnVectorProb(:,feature);
        pfunction = exp(x);
        denom = log(sum(pfunction));
  
        denominators = [denominators,denom];
    end
        
    for cluster = 1:size(lnVectorProb,1)
        for feature = 1:size(lnVectorProb,2)
            LnCompProb(cluster,feature) = lnVectorProb(cluster,feature) - denominators(1,feature);
        end
    end
...
% IMPLEMENT THIS FUNCTION (TASK A.c)
    ...
    for cluster = 1:size(LnCompProb,1)
        
        Np= sum(exp(LnCompProb(cluster,:)));

        weightC= Np/size(LnCompProb,2);
        model.weight(cluster) = weightC;
        
        meanC = 0;
        for feature = 1:size(LnCompProb,2)
            summand = trainVect(feature,:) * exp(LnCompProb(cluster,feature));
            meanC = meanC + summand;
        end
        meanC = (1/Np)*meanC;
        model.mean(cluster,:) = meanC;
                
        SigmaC = 0;
        for feature = 1:size(LnCompProb,2)
            d = trainVect(feature,:)'-meanC';
            summand = d*d'*exp(LnCompProb(cluster,feature));
            SigmaC = SigmaC + summand;
        end
        SigmaC = (1/Np)*SigmaC;
        model.covar(cluster,:,:) = SigmaC;
